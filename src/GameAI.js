...
const GameAI = () => {
  ...
  const handleMove = (index, player) => {
    if (board[index] !== null || finished) return; // Prevent move if the cell is taken or game is finished
    setBoard(prevBoard => {
  ...
  const checkWinner = (index, player) => {
    ...
    for (let i = 0; i < winCombinations.length; i++) {
      ...
      if (board[a] && board[a] === board[b] && board[a] === board[c]) {
        setText(player === "O" ? "CPU Wins!" : "You Win!");
        setFinished(true);
        if (player === "X") setWinner(true);
        return player;
      }
    }
    ...
    if (board.every(cell => cell !== null)) {
      ...
      setText(player === "X" ? "CPU's Turn" : "Your Turn");
    }
